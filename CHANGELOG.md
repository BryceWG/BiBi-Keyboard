...
- **AI 自动选择润色预设**：润色前可根据识别内容自动匹配最合适的 Prompt 预设；可为预设填写 Skill 描述，支持独立分类模型，并用历史记录预览效果；内容已足够好时也可自动跳过润色
- **录音时屏幕常亮**：键盘或悬浮球录音时可保持屏幕常亮，避免中途熄屏；也可通过键盘扩展按键开关
- **DashScope Qwen3.8-Omni-Flash**：DashScope 新增 Qwen3.8-Omni-Flash 非流式模型
- **StepAudio 新模型**：StepAudio 新增 stepaudio-3-asr-max，并支持自定义识别模型
- **[Pro] 助手与热词分析独立模型**：AI 助手和热词纠正分析可分别指定模型，不必跟随默认润色模型

### 改进优化

- **在线非流式分段识别**：在线文件识别也会按静音分段渐进识别，长录音更快出字；失败片段会自动重试
- **Prompt 全屏编辑**：润色 Prompt 支持全屏编辑
- **AI 更多参数**：温度等高级参数收入独立面板，设置页更清爽
- **[Pro] 输入框上下文**：开启后，自动选择润色预设也会参考光标前后文本

### 问题修复

- **折叠屏与旋转**：修复旋转或折叠后悬浮球、悬浮键盘位置与可用区域不准的问题
- **录音音源**：缓存已验证的录音源，减少部分机型录音失败或反复探测音源的问题
- **备份导入**：提高识别历史备份导入兼容性，避免新旧版本字段不兼容导致导入失败
...
- **深度思考字数阈值**：深度思考改为按识别字数启用。为 0 时始终开启，拉到上限则始终关闭，中间值仅在字数超过阈值时启用
- **Android 13 无障碍写入**：悬浮球可选使用新版无障碍 IME API 插入文字，终端和编辑器等场景更稳；开启后会关闭流式预览与写入兼容优化

### 改进优化

- **[Pro] 光标滑动**：滑动更跟手，多行上下移动更准；AI 编辑面板也可进入该模式

### 问题修复

- **识别历史丢失**：修复某些场景下识别记录丢失的问题
- **[Pro] 光标滑动**：修复动态键盘布局下无法进入滑动模式、滑到文本首尾时光标消失的问题
